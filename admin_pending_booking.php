...
if($role != 'admin'){
	header("Location: ./user_pending_booking");
}
...
$query = "SELECT * FROM (SELECT j.`#`, j.type, j.size,j.duration, j.price,j.ordered_by, j.confirmed_at, j.status, j.location,j.job_id, u.`#` as user_id, u.email, u.phone
FROM jobs AS j
JOIN users AS u ON j.ordered_by = u.email
ORDER BY j.confirmed_at ASC) AS a 
  ...
  <div class="uk-container">
    <div class="uk-card uk-card-small  uk-card-default">
      <div class="uk-card-header">
        <h3 class="uk-card-title">Pending Bookings</h3>
        <?php if ($error) {
      ...
      <div class="uk-card-body uk-padding-remove">
        <table class="uk-table uk-table-middle uk-table-responsive uk-table-divider uk-table-hover">
          ...
          <tbody>
            <?php
            if (!empty($confirmed_jobs)) {
              foreach ($confirmed_jobs as $key => $job) {
                $iter = $key + 1;
                ...
                echo "
						<tr>
							...
							<td>{$job['duration']}</td>
							<td>{$monied}</td>
							<td>{$job['confirmed_at']}</td>
							<td>{$job['phone']}</td>
              <td>{$job['location']}</td>
              <td>
              <div class=\"uk-button-group\">
              <a onclick=\"confirm_action(event, this)\" text class=\"uk-button uk-button-small uk-button-primary\"
                href=\"./complete_booking?job_id={$job['job_id']}&token={$token}&redirect=admin_pending_booking\"
              ...
              <a onclick=\"confirm_action(event, this)\" text class=\"uk-button uk-button-small uk-button-danger\"
                title=\"Delete Booking\" href=\"./delete_booking?job_id={$job['job_id']}&token={$token}&redirect=admin_pending_booking\"
              ...
              echo "
						<tr>
							<td class='uk-text-center' colspan='10'>No Data to Display</td>
							<tr>";
            }
            ?>
          </tbody>
