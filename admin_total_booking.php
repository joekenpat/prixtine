...
if ($role != 'admin') {
	header("Location: ./user_total_booking");
}
//load total user created jobs counts
$page = isset($_GET['page']) && is_numeric($_GET['page']) ? $_GET['page'] : 1;
$no_of_records_per_page = 10;
$offset = ($page - 1) * $no_of_records_per_page;
$query = "SELECT * FROM (SELECT j.`#`, j.type, j.size,j.duration, j.price,j.ordered_by, j.created_at, j.status, j.location,j.job_id, u.`#` as user_id, u.email, u.phone
FROM jobs AS j
JOIN users AS u ON j.ordered_by = u.email
ORDER BY j.created_at ASC) AS a 
			...
			<div class="uk-card-body uk-padding-remove">
						...
						if (!empty($total_jobs)) {
							foreach ($total_jobs as $key => $job) {
								...
								if ($job['status'] == 'confirmed') {
									$status = "<span class='uk-label'>{$job['status']}</span>";
									...
									<a onclick=\"confirm_action(event, this)\" text class=\"uk-button uk-button-small uk-button-primary\"
										href=\"./complete_booking?job_id={$job['job_id']}&token={$token}&redirect=admin_total_booking\"
										title=\"Complete Booking\" key=\"complete_btn_{$job['job_id']}\"
										id=\"complete_btn_{$job['job_id']}\" uk-icon=\"icon: check\">
									</a>
									<a onclick=\"confirm_action(event, this)\" text class=\"uk-button uk-button-small uk-button-danger\"
										title=\"Delete Booking\" href=\"./delete_booking?job_id={$job['job_id']}&token={$token}&redirect=admin_total_booking\"
										key=\"del_btn_{$job['job_id']}\" id=\"del_btn_{$job['job_id']}\" uk-icon=\"icon: trash\">
									</a>
								</div>";
								} elseif ($job['status'] == 'pending') {
									$status = "<span class='uk-label uk-label-warning'>{$job['status']}</span>";
									$action = "<div class=\"uk-button-group\">
              <a onclick=\"confirm_action(event, this)\" text class=\"uk-button uk-button-small uk-button-primary\"
                href=\"./complete_booking?job_id={$job['job_id']}&token={$token}&redirect=admin_total_booking\"
                title=\"Complete Booking\" key=\"complete_btn_{$job['job_id']}\"
              ...
              <a onclick=\"confirm_action(event, this)\" text class=\"uk-button uk-button-small uk-button-danger\"
                ...
                key=\"del_btn_{$job['job_id']}\" id=\"del_btn_{$job['job_id']}\" uk-icon=\"icon: trash\">
									...
									$status = "<span class='uk-label uk-label-success'>{$job['status']}</span>";
									$action = "<div class=\"uk-button-group\">
              <a onclick=\"confirm_action(event, this)\" text class=\"uk-button uk-button-small uk-button-danger\"
                title=\"Delete Booking\" href=\"./delete_booking?job_id={$message['job_id']}&token={$token}&redirect=admin_total_booking\"
                key=\"del_btn_{$job['job_id']}\" id=\"del_btn_{$job['job_id']}\" uk-icon=\"icon: trash\">
              </a>
            </div>";
								}
